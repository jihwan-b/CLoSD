...
import joblib
import time
from closd.utils.smpllib.smpllib.smpl_eval import compute_metrics_lite
...
class IMAMPPlayerContinuous(amp_players.AMPPlayerContinuous):
    def __init__(self, config):
        ...
        ### Guy
        self.has_mdm_task = hasattr(self.env.task, 'has_mdm_task') and self.env.task.has_mdm_task
        self.closd_eval = self.config['closd_eval']
    ...
    def _post_step(self, info, done):
        ...
        if self.closd_eval:
            ended_wo_terimation = torch.logical_and(done, 1-info['terminate'])
            successful_episods = torch.logical_and(ended_wo_terimation, info['task_done'])
            safe_num_resets = done.sum() + (done.sum() == 0)
            mdm_task_state_buf = info['task_state']
            # batch_dict['mdm_success_rate'] = (successful_episods.sum() / safe_num_resets).cpu().numpy()
            # batch_dict['mdm_success_per_task'] = {}
            for state in self.states_to_eval:
                if state == STATES.SIT:
                    state_resets = done & ((mdm_task_state_buf == STATES.SIT) | (mdm_task_state_buf == STATES.GET_UP))
                    state_success = (successful_episods & (mdm_task_state_buf == STATES.SIT)) | (done & (mdm_task_state_buf == STATES.GET_UP))  # i.e. sit done during the episode
                elif state in [STATES.STRIKE_KICK, STATES.STRIKE_PUNCH]:
                    state_resets = done & ((mdm_task_state_buf == STATES.STRIKE_KICK) | (mdm_task_state_buf == STATES.STRIKE_PUNCH) | (mdm_task_state_buf == STATES.HALT))
                    state_success = (successful_episods & (mdm_task_state_buf == STATES.STRIKE_KICK)) | (successful_episods & (mdm_task_state_buf == STATES.STRIKE_PUNCH)) | (done & (mdm_task_state_buf == STATES.HALT))  # i.e. strike done during the episode
                    # FIXME - calc mutual success rate for both tasks since in the HALT state we don't have info what was the original one
                else:
                    state_resets = done & (mdm_task_state_buf == state)
                    state_success = successful_episods & (mdm_task_state_buf == state)
                self.success_counters[state]['resets'] += int(state_resets.sum())
                self.success_counters[state]['success'] += int(state_success.sum())
            
            if all([self.success_counters[k]['resets'] >= self.data_points_per_task for k in self.states_to_eval]):
                print(f'Collected enough data ({self.data_points_per_task} per task) success rates:')
                for state in self.states_to_eval:
                    print('{}: {:.2f}'.format(str(state), self.success_counters[state]['success']/self.success_counters[state]['resets']))
                print('Eval plot ended')
                exit()
    ...
    def run(self):
        ...
        for t in range(n_games):
            if games_played >= n_games or getattr(self.env.task, 'done_saving_episodes', False):
                break
